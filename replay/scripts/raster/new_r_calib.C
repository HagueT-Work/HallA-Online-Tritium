...
void new_r_calib(){
  ...
  TCanvas *ytar_canvas = new TCanvas("y Target Canvas");
  ytar_canvas->Divide(2,1);
  ytar_canvas->cd(1);

  TCut good = "";

  TProfile *ytar_profile_1 = new TProfile("ytar_profile_1","Average y target vs Raster 1 X",20,45000,95000,-0.2,0.2);
  rootfile->Draw(det + ".tr.tg_y:" + arm + ".Raster.rawcur.x>>ytar_profile_1",good);

  ytar_canvas->cd(2);
  TProfile *ytar_profile_2 = new TProfile("ytar_profile_2","Average y target vs Raster 2 X",20,45000,95000,-0.2,0.2);
  rootfile->Draw(det + ".tr.tg_y:" + arm + ".Raster2.rawcur.x>>ytar_profile_2",good);
